...
from cms import rmtree, utf8_decoder, config
from cms.db import (
...
class DumpExporter:
    ...
    def __init__(
        ...
        if temp_dir is not None:
            ...
            # Use config.temp_dir as default (consistent with rest of CMS)
    ...
    def do_export(self):
        ...
        if archive_info["write_mode"] != "":
            ...
            # Check available space in temporary directory before starting
            ...
            try:
                ...
                # Warning if temp dir is likely to be problematic
                if temp_base == "/tmp" and free_space_gb < 8:
                    logger.warning(
                        "Using /tmp for temporary files with only %.1f GB free space. "
                        "On systems where /tmp is mounted in RAM (like Arch Linux), "
                        "this may cause system freezes. Consider using --temp-dir "
                        "to specify a different location with more space, or set "
                        "TMPDIR environment variable.", free_space_gb)
                elif free_space_gb < 2:
                    logger.warning(
                        "Temporary directory %s has only %.1f GB free space. "
                        "Consider using --temp-dir to specify a location with more space.",
                        temp_base, free_space_gb)
                        
            except (OSError, AttributeError):
                # If we can't check space, just warn about potential issues
                if temp_base == "/tmp":
                    logger.warning(
                        "Using /tmp for temporary files. On systems where /tmp is "
                        "mounted in RAM (like Arch Linux), this may cause system "
                        "freezes if the export is large. Consider using --temp-dir "
                        "to specify a different location, or set TMPDIR environment variable.")
            
            export_dir = os.path.join(tempfile.mkdtemp(dir=self.temp_dir),
...
def main():
    ...
    parser.add_argument("--temp-dir", action="store", type=str,
                        help="directory to use for temporary files "
                             "(default: system temp dir configured in CMS)")
